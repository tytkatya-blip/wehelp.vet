...
export default function InsightSection() {
  return <section className="insight-section" aria-labelledby="insight-title"><Container>
    ...
    <div className="editorial-rows">{insights.map(([title,text]) => <article className="split-grid editorial-row" key={title}><div className="insight-label"><img src="/media/attention-icon.svg" width="40" height="40" alt="" loading="lazy" /><h3>{title}</h3></div><p>{text}</p></article>)}</div>
  </Container></section>
}
